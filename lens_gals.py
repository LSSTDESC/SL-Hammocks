...
def galaxy_size(mh, mstar, z, cosmo, q_out="tb", model="oguri20", scatter=False, sig_tb=0.1):
    ...
    if q_out == "tb":
        convert_t = 1.0 / cosmo.angularDiameterDistance(z) * 206264.8
...
def modelOguri20(mh, z):
    ...
    -----------------------------------------------------------------------------------------------
    mh: ndarray
        (sub)halo mass [M_sol/h]; can be a number or a numpy array.
    z:  ndarray
...
def modelscVanderwel23(mstar, n):
    ...
    These values are determined by applying the curve_fit function in scipy.optimize
    to the stellar half-mass radii data of the 84% and 16% percentiles in Table 5
    of van der Wel et al. 2023 (arXiv: 2307.03264), which are for quiescent galaxies at 0.5 < z < 1.0.
...
def gals_init(TYPE_SMHM="true"):
    ...
    The values with TYPE_SMHM == "true" show the true values for quenched galaxies
    from Markov Chain Monte Carlo calculation in P. Behroozi et al. 2019
...
def galaxy_properties(mh_vir, zl, p_h, pa_smhm, frac_SM_IMF, TYPE_GAL_SIZE, sig_mg, sig_tb=0.2):
    """
    Calculate galaxy properties such as stellar mass, half-light radius, ellipticity, and position angle.

    Parameters
    -----------------------------------------------------------------------------------------------
    :param mh_vir: Virial masses of the halos in units of M_sol/h
    :type  mh_vir: array_like

    :param zl: The redshift of the galaxies.
    :type  zl: float

    :param p_h: Position angles of the halos, in degrees.
    :type  p_h: ndarray

    :param pa_smhm: Parameters for the stellar mass-halo mass relation.
    :type  pa_smhm: dict or similar structure

    :param frac_SM_IMF: Fraction of stellar mass according to the Initial Mass Function.
    :type  frac_SM_IMF: float

    :param TYPE_GAL_SIZE: Type of galaxy size model to use.
    :type  TYPE_GAL_SIZE: str

    :param sig_mg: Standard deviation of lognormal scatter applied to galaxy stellar mass.
    :type  sig_mg: float

    :param sig_tb: Standard deviation of lognormal scatter applied to oguri20 model (optional, default is 0.2).
    :type  sig_tb: float

    Returns
    -----------------------------------------------------------------------------------------------
    m_g: ndarray
        Stellar masses of galaxies with applied scatter in units of M_sol/h

    tb_g: ndarray
        Half-light radii of galaxies in units of arcsec

    e_g: ndarray
        Ellipticities of the galaxies.

    p_g: ndarray
        Position angles of the galaxies, in degrees.
    """
    cosmo = cosmology.getCurrent()
    hubble = cosmo.H0 / 100.0
    m_g = stellarmass_halomass(mh_vir / hubble, zl, pa_smhm, frac_SM_IMF) * hubble
    m_g = m_g * np.random.lognormal(0.0, sig_mg)
    e_g, p_g = set_gals_param(p_h)
    tb_g = galaxy_size(mh_vir, m_g / frac_SM_IMF, zl, cosmo, model=TYPE_GAL_SIZE, scatter=True, sig_tb=sig_tb)
    return m_g, tb_g, e_g, p_g
